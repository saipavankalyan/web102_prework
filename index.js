...
function addGamesToPage(games) {
    ...
    games.forEach(game => {
        ...
        gamecardElement.innerHTML = `<p> Game name: ${game.name} </p>
        <p> Goals in Game: ${game.goal} </p>
        <p> ${game.name} is funded by ${game.backers} investors </p>
        <img class="game-img" src="${game.img}" alt="Image of Game" />`

         // append the game to the games-container
        document.querySelector("#games-container").appendChild(gamecardElement);
    });
}
...
const totalGamesPlayed = GAMES_JSON.reduce((acc, game) => {
    return acc + 1;
}, 0);

gamesCard.innerHTML = `${totalGamesPlayed.toLocaleString('en-US')}`;
